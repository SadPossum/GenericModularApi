...
| Projection rebuild tasks | Pending | Add production-safe projection rebuild/backfill support for derived read models. The requirements live in [Projection Rebuild Tasks](projection-rebuild-tasks.md). Keep rebuild tasks explicit, resumable, tenant-safe, observable, and guarded against hidden cross-module internals. |
