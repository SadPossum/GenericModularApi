...
| Background worker host | Pending | Add an optional `Host.Worker` process for production deployments that should keep HTTP hosts request-focused while separately running outbox publishing, NATS consumers, inbox/projection handlers, and TaskRuntime workers. See [Background Worker Host Task](background-worker-host-task.md). |
...
| Shared access policy foundation | Pending | Add a small `Shared.AccessControl` contracts package for resource-level policy vocabulary and decisions. Keep existing admin RBAC unchanged, keep business rules module-owned, and defer persisted grants or external policy engines until repeated module needs prove the shape. |
